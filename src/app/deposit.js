...
async function showPaymentDetails(pM) {
    try {
        let currency =
            pM === "Ethereum" ? "ETH" : pM === "Bitcoin" ? "BTC" : "USDT";
        ...
        const randomAddress =
            allAddresses[Math.floor(Math.random() * allAddresses.length)];
...
function copyToClipboard(e) {
    ...
    textarea.value = document.querySelector("#paymentInput").value;
    // textarea.value =
    //     ADDRESSES[
    //         `${String(
    //             paymentMethod[paymentMethod.value].textContent
